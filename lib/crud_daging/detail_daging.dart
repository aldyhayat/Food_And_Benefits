...
import 'package:food/crud_daging/update_daging.dart';
import 'package:http/http.dart' as http;
import 'dart:convert';

class DetailDaging extends StatefulWidget {
  List list;
  int index;
  DetailDaging({required this.list,required this.index});

  @override
  State<DetailDaging> createState() => _DetailDagingState();
}

class _DetailDagingState extends State<DetailDaging> {
  void deleteData(){
    var url=Uri.parse("http://192.168.254.107/food_fullstack/api_food/delete_daging.php");
    http.post(url, body: {
        ...
        new RaisedButton(
          ...
          onPressed: ()=> {
            ...
            Navigator.of(context).push(
              ...
              (BuildContext context)=> new ListDaging())
            );
            })
            },
          ),
                  ...
                  children: [
                    Container(
                      ...
                      decoration: BoxDecoration(
                        image: DecorationImage(image: NetworkImage("http://192.168.254.107/food_fullstack/api_food/uploads_daging/${widget.list[widget.index]['foto_makanan']}"), fit: BoxFit.fill),
                      ),       
                    ),
                    SizedBox(height: 20,),
                    Padding(
                      ...
                      children: [
                        RaisedButton(
                            onPressed: ()=>Navigator.of(context).push(
                              new MaterialPageRoute(builder: (BuildContext context) =>  UpdateDaging(list: widget.list, index: widget.index,))
                            ),
