...
import 'package:food/crud_minuman/update_minuman_admin.dart';
import 'package:http/http.dart' as http;
import 'dart:convert';
class DetailMinumanAdmin extends StatefulWidget {
  List list;
  int index;
  DetailMinumanAdmin({required this.list,required this.index});

  @override
  State<DetailMinumanAdmin> createState() => _DetailMinumanAdminState();
}

class _DetailMinumanAdminState extends State<DetailMinumanAdmin> {
  void deleteData(){
    var url=Uri.parse("http://192.168.254.107/food_fullstack/api_food/delete_minuman.php");
    http.post(url, body: {
        ...
        new RaisedButton(
          ...
          onPressed: ()=> {
            ...
            Navigator.of(context).push(
              ...
              (BuildContext context)=> new ListMinumanAdmin())
            );
            })
            },
          ),
                  ...
                  children: [
                    Container(
                      ...
                      decoration: BoxDecoration(
                        image: DecorationImage(image: NetworkImage("http://192.168.254.107/food_fullstack/api_food/uploads_minuman/${widget.list[widget.index]['foto_makanan']}"), fit: BoxFit.fill),
                      ),       
                    ),
                    SizedBox(height: 20,),
                    Padding(
                      ...
                      children: [
                        RaisedButton(
                            onPressed: ()=>Navigator.of(context).push(
                              new MaterialPageRoute(builder: (BuildContext context) =>  UpdateMinumanAdmin(list: widget.list, index: widget.index,))
                            ),
